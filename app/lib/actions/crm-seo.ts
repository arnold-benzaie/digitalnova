...
async function getWebsiteOrThrow(websiteId: string) {
...
export async function runSeoAudit(websiteId: string) {
  const website = await getWebsiteOrThrow(websiteId);
...
export async function updateSeoIssueStatus(issueId: string, status: string) {
  if (!SEO_ISSUE_STATUS_VALUES.includes(status)) throw new Error("Statut invalide.");
  ...
  const [issue] = await db
    ...
    .returning();
  if (!issue) throw new Error("Recommandation introuvable.");

  const [audit] = await db.select().from(seoAudits).where(eq(seoAudits.id, issue.auditId)).limit(1);
  const website = audit ? await getWebsiteOrThrow(audit.websiteId) : null;
...
export async function addSeoKeyword(formData: FormData) {
  ...
  if (typeof websiteId !== "string" || !websiteId) throw new Error("Site requis.");
  ...
  if (!row) throw new Error("Ce mot-clé est déjà suivi pour ce site.");
...
export async function deleteSeoKeyword(id: string) {
  ...
  if (!existing) throw new Error("Mot-clé introuvable.");
  const website = await getWebsiteOrThrow(existing.websiteId);
...
export async function refreshKeywordRankings(websiteId: string) {
  ...
  if (keywords.length === 0) throw new Error("Aucun mot-clé suivi pour ce site.");
