...
function requiredString(formData: FormData, field: string, label: string): string {
...
export async function createProspectBusinessAndAudit(formData: FormData) {
  ...
  const firstName = requiredString(formData, "firstName", "Le prénom du prospect");
  const lastName = requiredString(formData, "lastName", "Le nom du prospect");
  const legalName = requiredString(formData, "legalName", "Le nom officiel de l'entreprise");
...
export async function updateAuditStatus(auditId: string, status: string) {
  ...
  if (!current) throw new Error("Audit introuvable.");
  if (!FREELY_SETTABLE_STATUSES.includes(current.status)) {
    throw new Error("Cet audit est déjà engagé dans le circuit de validation — utilisez les actions de l'onglet Rapport.");
  }
...
export async function updateBusinessProfileStatus(auditId: string, businessId: string, status: string) {
  ...
  if (!PROFILE_STATUS_VALUES.includes(status)) {
    throw new Error("Statut de profil invalide.");
  }
