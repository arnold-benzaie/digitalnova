...
test("integration: dry-running the engine against all 15 real db/migrations files produces a clean, fully-preview-qualified plan", async () => {
  ...
  assert.equal(result.statements[0].sql, `CREATE SCHEMA IF NOT EXISTS "preview"`);
  assert.equal(result.statements[1].sql, `SET LOCAL search_path TO "preview"`);
  assert.ok(result.statementCount > 10, "expected well more than one statement per file across 15 migrations");
