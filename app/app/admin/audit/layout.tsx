...
/**
 * Scoped to /admin/audit/* only — mounts the ⌘K command palette and the
 * audit-specific notification bell once for the whole module without
 ...
 *
 * Does NOT mount its own toast host anymore (components/gbp-audit/ui/
 * toast.tsx's AuditToaster) — this layout nests inside the shared
 * app/admin/layout.tsx -> AppShell -> AppShellClient tree, which already
 * mounts one (components/notification-toaster.tsx, styled to match
 * AuditToaster's success/error/warning variants too). sonner's toast() is
 * a global singleton: a second <Toaster/> here rendered every toast.*()
 * call TWICE on audit pages, not scoped separately as it might appear.
 */
