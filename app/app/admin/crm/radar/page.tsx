...
import { getRadarQueue } from "@/lib/actions/radar-queue";
...
function buildHref(priority: Priority | undefined, page: number | undefined) {
...
type Params = { priority?: string; page?: string };
...
export default async function CrmRadarPage({ searchParams }: { searchParams: Promise<Params> }) {
  ...
  const result = await getRadarQueue({ page, priority: priority ? [priority] : undefined });
  ...
  // totalQualified is computed BEFORE the priority filter (see
  // lib/actions/radar-queue.ts) — safe to use for "Page X of Y" only when
  // no filter narrows the displayed population. With a filter active, the
  // API cannot truthfully report a filtered total page count, so pagination
  // falls back to a Previous/Next-only heuristic (Next enabled only when a
  // full page came back) rather than guessing.
  const totalPages = Math.max(1, Math.ceil(result.totalQualified / result.pageSize));
  const showPageCount = !priority;
  const hasPrevious = page > 1;
  const hasNext = priority ? result.items.length === result.pageSize : page < totalPages;
        ...
        <>
          ...
          {(hasPrevious || hasNext) && (
            <div className="mt-4 flex items-center justify-between text-sm">
              <Link
                href={buildHref(priority, page > 1 ? page - 1 : undefined)}
              ...
              <Link
                href={buildHref(priority, hasNext ? page + 1 : undefined)}
