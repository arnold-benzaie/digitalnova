...
export function EditTaskForm({ task, locale = "fr" }: { task: Task; locale?: Locale }) {
  const router = useRouter();
  const dialogRef = useRef<HTMLDialogElement>(null);
  const [isPending, startTransition] = useTransition();
  const [error, setError] = useState<string | null>(null);
  const dueDate = task.dueDate ? new Date(task.dueDate).toISOString().slice(0, 10) : "";
  const t = dictionaries[locale].crm.tasks.edit;
...
export function DeleteTaskButton({ id, locale = "fr" }: { id: string; locale?: Locale }) {
  const router = useRouter();
  const [isPending, startTransition] = useTransition();
  const [error, setError] = useState<string | null>(null);
  const t = dictionaries[locale].crm.tasks.delete;
